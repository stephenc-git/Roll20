...
var DeathTracker = DeathTracker || (function () {
    ...
    const styles = {
                    ...
                    case 'config':
                        ...
                        if (args.length > 0) {
                            ...
                            if (key === 'death_statusmarker') {
                                if (value !== state[state_name].config.twothird_statusmarker) {
                                    state[state_name].config[key] = value;
                                } else {
                                    message = '<span style="color: red">Statusmakers can\'t be the same.</span>';
                                }
                            } else if (key === 'twothird_statusmarker') {
                                if (value !== state[state_name].config.death_statusmarker) {
                                    state[state_name].config[key] = value;
                                } else {
                                    message = '<span style="color: red">Statusmakers can\'t be the same.</span>';
                                }
                            } else if (key === 'threethird_statusmarker') {
                                if (value !== state[state_name].config.death_statusmarker) {
                                    state[state_name].config[key] = value;
                                } else {
                                    message = '<span style="color: red">Statusmakers can\'t be the same.</span>';
                                }
                            } else if (key === 'onethird_statusmarker') {
                                if (value !== state[state_name].config.death_statusmarker) {
                                    state[state_name].config[key] = value;
                                } else {
                                    message = '<span style="color: red">Statusmakers can\'t be the same.</span>';
        ...
        handleBarValueChange = (obj, prev) => {
            ...
            let onethirdMarker = state[state_name].config.onethird_statusmarker;
            let twothirdMarker = state[state_name].config.twothird_statusmarker;
            let threethirdMarker = state[state_name].config.threethird_statusmarker;
            ...
            if (deathMarker !== 'none' && obj.get(bar + '_value') <= 0) {
                ...
                attributes['status_' + onethirdMarker] = false;
                attributes['status_' + twothirdMarker] = false;
                attributes['status_' + threethirdMarker] = false;
            } else {
                attributes['status_' + deathMarker] = false;
                attributes['status_' + unconsciousMarker] = false;
				attributes['status_' + onethirdMarker] = (onethirdMarker !== 'none' && obj.get(bar + '_max') !== '' && obj.get(bar + '_value') <= obj.get(bar + '_max') * 0.30 );
				
                attributes['status_' + twothirdMarker] = (twothirdMarker !== 'none' && obj.get(bar + '_max') !== '' && obj.get(bar + '_value') <= obj.get(bar + '_max') * 0.60 && obj.get(bar + '_value') > obj.get(bar + '_max') * 0.30 );
				
				attributes['status_' + threethirdMarker] = (threethirdMarker !== 'none' && obj.get(bar + '_max') !== '' && obj.get(bar + '_value') <= obj.get(bar + '_max') * 0.90 && obj.get(bar + '_value') > obj.get(bar + '_max') * 0.60 );
				
				log(obj.get(bar + '_max') * 0.90);
            }
            ...
            if(state[state_name].config.change_player_aura && isPlayer || state[state_name].config.change_npc_aura && !isPlayer){
                let color = getColor(1 - (obj.get(bar + '_value') / obj.get(bar + '_max')));
                attributes[`aura1_color`] = color;
                attributes[`aura1_radius`] = 0.7;
                attributes[`aura2_color`] = '#000000';
                attributes[`aura2_radius`] = 0.01;
            }
        ...
        sendConfigMenu = (first, message) => {
            ...
            let death_markerButton = makeButton(state[state_name].config.death_statusmarker, '!' + state[state_name].config.command + ' config death_statusmarker|' + markerDropdown, styles.button + styles.float.right),
                twothird_markerButton = makeButton(state[state_name].config.twothird_statusmarker, '!' + state[state_name].config.command + ' config twothird_statusmarker|' + markerDropdown, styles.button + styles.float.right),
				threethird_markerButton = makeButton(state[state_name].config.threethird_statusmarker, '!' + state[state_name].config.command + ' config threethird_statusmarker|' + markerDropdown, styles.button + styles.float.right),
				onethird_markerButton = makeButton(state[state_name].config.onethird_statusmarker, '!' + state[state_name].config.command + ' config onethird_statusmarker|' + markerDropdown, styles.button + styles.float.right),
                ...
                change_player_auraButton = makeButton(state[state_name].config.change_player_aura, '!' + state[state_name].config.command + ' config change_player_aura|' + !state[state_name].config.change_player_aura, styles.button + styles.float.right),
                change_npc_auraButton = makeButton(state[state_name].config.change_npc_aura, '!' + state[state_name].config.command + ' config change_npc_aura|' + !state[state_name].config.change_npc_aura, styles.button + styles.float.right),
                ...
                listItems = [
                    ...
                    '<span style="'+styles.float.left+'">Injured Statusmarker:</span> ' + threethird_markerButton,
                    '<span style="'+styles.float.left+'">Hurt Statusmarker:</span> ' + twothird_markerButton,
                    '<span style="'+styles.float.left+'">Raugh Statusmarker:</span> ' + onethird_markerButton,
                    ...
                    '<span style="'+styles.float.left+'">Change Player Aura Color:</span> ' + change_player_auraButton,
                    '<span style="'+styles.float.left+'">Change NPC Aura Color:</span> ' + change_npc_auraButton,
        ...
        setDefaults = (reset) => {
            const defaults = {
                config: {
					...
					threethird_statusmarker: 'green',
                    twothird_statusmarker: 'yellow',
					onethird_statusmarker: 'red',
                    bar: 1,
                    firsttime: (reset) ? false : true,
                    pc_unconscious_statusmarker: 'sleepy',
                    change_player_tint: true,
                    change_npc_tint: true,
                    change_player_aura: true,
                    change_npc_aura: true,
                    fx: true,
            ...
            if (!state[state_name].config) {
                ...
                if (!state[state_name].config.hasOwnProperty('twothird_statusmarker')) {
                    state[state_name].config.twothird_statusmarker = defaults.config.twothird_statusmarker;
                }
                if (!state[state_name].config.hasOwnProperty('onethird_statusmarker')) {
                    state[state_name].config.onethird_statusmarker = defaults.config.onethird_statusmarker;
                }
                if (!state[state_name].config.hasOwnProperty('threethird_statusmarker')) {
                    state[state_name].config.threethird_statusmarker = defaults.config.threethird_statusmarker;
                }
                ...
                if (!state[state_name].config.hasOwnProperty('change_player_aura')) {
                    state[state_name].config.change_player_aura = defaults.config.change_player_aura;
                }
                if (!state[state_name].config.hasOwnProperty('change_npc_aura')) {
                    state[state_name].config.change_npc_aura = defaults.config.change_npc_aura;
                }
